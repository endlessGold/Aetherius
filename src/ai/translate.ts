...
import type { ControlService } from './llmService.js';
...
/**
 * 영어 텍스트를 한국어로 번역한다. ControlService 호출 1회.
 * 실패 시 원문을 그대로 반환한다.
 */
export async function translateToKorean(control: ControlService, text: string): Promise<string> {
  if (!text || !text.trim()) return text;
  try {
    const out = await control.generateResponse(TRANSLATE_PROMPT_PREFIX + text, TRANSLATE_SYSTEM);
