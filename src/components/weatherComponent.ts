...
export class WeatherComponent extends ComponentBase<WeatherState> {
  ...
  private updateWeather(payload: Partial<WeatherState>) {
    ...
    switch (this.state.condition) {
      case 'Sunny':
        if (payload.sunlightIntensity === undefined) this.state.sunlightIntensity = 100;
        if (payload.precipitation === undefined) this.state.precipitation = 0;
        break;
      case 'Rainy':
        if (payload.sunlightIntensity === undefined) this.state.sunlightIntensity = 40;
        if (payload.humidity === undefined) this.state.humidity = 80;
        if (payload.precipitation === undefined) this.state.precipitation = 10;
        break;
      case 'Stormy':
        if (payload.sunlightIntensity === undefined) this.state.sunlightIntensity = 20;
        if (payload.windSpeed === undefined) this.state.windSpeed = 15;
        if (payload.precipitation === undefined) this.state.precipitation = 50;
        break;
      case 'Cloudy':
        if (payload.sunlightIntensity === undefined) this.state.sunlightIntensity = 60;
        break;
      case 'Drought':
        if (payload.precipitation === undefined) this.state.precipitation = 0;
        if (payload.humidity === undefined) this.state.humidity = 10;
        if (payload.temperature === undefined) this.state.temperature = 35;
        break;
    }
  }
}
