...
export interface PlantState {
  ...
  age: number;              // days
  health: number;           // 0-100
  maturityAge: number;      // days
  alive: boolean;

  // 2. Environmental Impact Variables (Current State)
  waterContent: number;     // %
  ...
  // 3. Physiological Parameters (Species Traits)
  growthRateBase: number;       // cm/day
  photosynthesisRate: number;   // efficiency multiplier
  respirationRate: number;      // energy cost
  waterUptakeEfficiency: number;
  nutrientUptakeEfficiency: number;
  lightUseEfficiency: number;

  // 4. Interaction & Competition
  shadingFactor: number;        // 0-1 (1 = full shade impact)
  competitionPressure: number;  // 0-1
  herbivoryPressure: number;    // 0-1

  // 5. Stress & Disease
  droughtStress: number;        // 0-1
  frostStress: number;          // 0-1
...
export class PlantComponent implements Component {
  ...
  private processGrowth(env: any) {
    ...
    if (this.state.health < 50) growth *= 0.5;
