...
export namespace Environment {
  ...
  export class WeatherChange extends Event<{ layer: number; delta: number; x: number; y: number }> {
    constructor(layer: number, delta: number, x: number, y: number) { super({ layer, delta, x, y }); }
  }
  export class GlobalParameterChange extends Event<{ layer: number; delta: number }> {
    constructor(layer: number, delta: number, sourceId?: string) { super({ layer, delta }, sourceId, 2); }
  }
}
...
export namespace Biological {
  ...
  export class EntityMoved extends Event<{
    ...
    constructor(entityId: string, from: { x: number; y: number; placeId?: string }, to: { x: number; y: number; placeId?: string }, reason: string, sourceId?: string) {
  ...
  export class InfectionExposed extends Event<{ entityId: string; strainId: string; load: number }> {
    constructor(entityId: string, strainId: string, load: number, sourceId?: string) { super({ entityId, strainId, load }, sourceId, 1); }
  }
  export class InfectionContracted extends Event<{ entityId: string; strainId: string }> {
    constructor(entityId: string, strainId: string, sourceId?: string) { super({ entityId, strainId }, sourceId, 1); }
  }
  ...
  export class DiedOfDisease extends Event<{ entityId: string; strainId: string }> {
    constructor(entityId: string, strainId: string, sourceId?: string) { super({ entityId, strainId }, sourceId, 2); }
  }
...
export namespace Interaction {
  ...
  export class Eat extends Event<{ targetId: string; amount: number }> {
    constructor(targetId: string, amount: number, sourceId?: string) { super({ targetId, amount }, sourceId); }
  }
  export class Attack extends Event<{ targetId: string; damage: number }> {
    constructor(targetId: string, damage: number, sourceId?: string) { super({ targetId, damage }, sourceId); }
  }
  export class Mate extends Event<{ targetId: string }> {
    constructor(targetId: string, sourceId?: string) { super({ targetId }, sourceId); }
  }
  export class Communicate extends Event<{ targetId?: string; message: string }> {
    constructor(message: string, targetId?: string, sourceId?: string) { super({ targetId, message }, sourceId); }
  }
...
export namespace System {
  ...
  export class Tick extends Event<{ tickCount: number; deltaTime: number; environment?: Record<string, any> }> {
    constructor(tickCount: number, deltaTime: number, environment?: Record<string, any>) { super({ tickCount, deltaTime, environment }); }
  }
  ...
  export class AIAgentSense extends Event<{
    ...
    constructor(payload: { targetId: string; bioState: { hunger: number; energy: number; stress: number; health: number }; actionParams: Record<string, any>; environment?: Record<string, any> }) {
      super(payload);
    }
  }
  export class ChangeWeather extends Event<Record<string, any>> {
    constructor(payload: Record<string, any>) { super(payload, undefined, 1); }
  }
  export class AsyncRequest extends Event<{
    requestId: string; action: string; params: any;
    resolve: (value: any) => void; reject: (reason: any) => void;
  }> {
    constructor(payload: { requestId: string; action: string; params: any; resolve: (value: any) => void; reject: (reason: any) => void }) {
  ...
  export class WormholeClosed extends Event<{ wormholeId: string; a: string; b: string }> {
    constructor(wormholeId: string, a: string, b: string, sourceId?: string) { super({ wormholeId, a, b }, sourceId, 2); }
  }
...
export namespace Economy {
  export class Event<T = unknown> extends Simulation.Event<T> {
    constructor(payload: T, sourceId?: string, priority: number = 1) {
  ...
  export class DefaultOccurred extends Event<{ entityId: string; rehabCount: number }> {
    constructor(entityId: string, rehabCount: number, sourceId?: string) {
...
export namespace Command {
  ...
  export class EntityCreateRequested extends Event<{ id: string; assemblyType: string }> {
    constructor(id: string, assemblyType: string, sourceId?: string) { super({ id, assemblyType }, sourceId, 2); }
  }
