...
## 10) tools/ 및 GUI·HTML 규칙

- **tools/** 디렉터리
  - `tools/` 아래 코드는 운영/실험/데이터 처리용 스크립트만 둔다.
  - 도메인 로직은 `src/`에 두고, `tools/`에서는 가능한 한 기존 모듈을 import 해서 재사용한다.
  - Node 전용 코드(파일 시스템, 프로세스 제어 등)는 `tools/`에 국한하고, 시뮬레이션 코어(`src/core`, `src/entities` 등)로 역류하지 않게 유지한다.

- **GUI 도구**
  - GUI 기반 도구가 필요할 경우, 항상 **컴포넌트 기반 GUI 방식**을 채택한다.
  - 예: React/컴포넌트 트리/상태 관리 등으로 화면을 구성하고, 직접 DOM을 수동 조작하는 방식은 피한다.

- **HTML + Script 태그**
  - 정적 HTML에서 `<script>` 태그 안에 **직접 JavaScript 코드를 작성하지 않는다.**
  - `<script src="...">` 또는 ES 모듈 번들만 로드하고, 실제 로직은 별도 JS/TS 모듈에 둔다.
  - `onclick="..."` 같은 인라인 이벤트 핸들러를 쓰지 않고, 컴포넌트/프레임워크 측에서 이벤트를 바인딩한다.

- ** 에이전트 코드작성시의 GUI 코드 의존성**
  - GUI를 작성할 때, 가능하면 **내장 전용 라이브러리 대신 표준 웹 API·레퍼런스·웹 검색 기반 호출**을 선호한다.
  - 특정 벤더 SDK에 강하게 고정되기보다는, HTTP/웹 표준을 우선으로 두고 필요 시 최소한의 래퍼만 추가한다.
