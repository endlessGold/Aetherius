...
export class CommandHandler {
    constructor(world, weatherEntity) {
        ...
        this.world.eventBus.subscribe(System.ChangeWeather, (event) => {
            ...
            const components = weatherBehavior?.components;
    ...
    async handleWarpEvolution(args) {
    ...
    handleSmite(args) {
        ...
        if (isNaN(x) || isNaN(y))
            return { success: false, message: "Usage: smite <x> <y> [radius]" };
        // Apply devastation
    ...
    handleBless(args) {
        ...
        this.getManager().entities.forEach(entity => {
            ...
            if (behavior && behavior.components && behavior.components.vitality) {
                ...
                if (target === 'plants' && behavior.components.growth)
                    match = true;
                if (target === 'creatures' && behavior.components.goalGA)
                    match = true;
                if (match) {
                    behavior.components.vitality.hp = 100;
                    if (behavior.components.energy)
                        behavior.components.energy.energy = 100;
                    count++;
                }
            }
        });
    ...
    handleMeteor(args) {
        ...
        const y = parseFloat(args[1]);
        if (isNaN(x) || isNaN(y))
            return { success: false, message: "Usage: meteor <x> <y>" };
        // Massive impact
        const radius = 15;
    ...
    handleOracle() {
        ...
        let advice = "";
        if (entityCount < 10)
            advice = "The world is barren. Consider 'spawn_entity' or 'bless'.";
        else if (entityCount > 200)
            advice = "Life teems uncontrollably. A 'smite' might restore balance.";
        else
            advice = "The world is in harmony. Watch and wait.";
        return {
            success: true,
    ...
    handleWatch(args) {
        const targetId = args[0];
        if (!targetId)
            return { success: false, message: "Usage: watch <id>" };
        const entity = this.getManager().getEntity(targetId);
        if (!entity)
            return { success: false, message: "Entity not found." };
        // Get component state
        ...
        if (comp.vitality)
            info += `HP: ${comp.vitality.hp.toFixed(1)} | `;
        if (comp.energy)
            info += `Energy: ${comp.energy.energy.toFixed(1)} | `;
        if (comp.position)
            info += `Pos: (${comp.position.x.toFixed(1)}, ${comp.position.y.toFixed(1)})\n`;
        if (comp.goalGA) {
    ...
    handleAutoGod(args) {
        ...
        const autoGod = this.world.autoGodSystem;
        if (!autoGod)
            return { success: false, message: "Auto God System not initialized." };
        if (state === 'on') {
            autoGod.toggle(true);
    ...
    handleExploreLoc(args) {
        const mazeSystem = this.world.mazeSystem;
        if (!mazeSystem)
            return { success: false, message: "Maze System not active." };
        const network = mazeSystem.network;
        const count = network.nodes.size;
        const active = Array.from(network.nodes.values()).filter((n) => n.maze.activityLevel > 10).length;
    ...
    async handleAskScience(args) {
        const query = args.join(' ');
        if (!query)
            return { success: false, message: "Usage: ask_science <question>" };
        try {
    ...
    buildScienceContext() {
        ...
        const plants = entities.filter((e) => {
        ...
        for (const e of entities) {
            ...
            if (!c)
                continue;
            if (c.vitality?.hp != null)
                totalHp += c.vitality.hp;
            if (c.energy?.energy != null)
                totalEnergy += c.energy.energy;
            counted++;
        }
        const maze = this.world.mazeSystem?.network;
        const nodeCount = maze?.nodes?.size ?? 0;
        const edgeCount = maze
            ? Array.from(maze.nodes.values()).reduce((sum, n) => sum + (n?.maze?.connections?.size ?? 0), 0) / 2
            : 0;
    ...
    renderScienceReportMarkdown(report) {
        const header = `\n📄 [Aetherius Science Report]\n- Tick: ${this.world.tickCount}\n- Query: ${report.query}\n\n`;
        const agents = report.hypotheses
            .map((h) => `## ${h.agent}\n${h.content}\n`)
            .join('\n');
        const reviews = report.reviews
            .map((r) => `- ${r.reviewer} → ${r.target}\n${r.critique}\n`)
            .join('\n');
        const synthesis = `# Synthesis\n${report.synthesis}\n`;
        return `${header}# Context\n${report.projectContext}\n\n# Hypotheses\n${agents}\n# Peer Review\n${reviews}\n${synthesis}`;
    }
    async appendScienceReportToFile(entry) {
    ...
    handleAiEvents(args) {
        const state = args[0]?.toLowerCase();
        const sys = this.world.aiEventOrchestratorSystem;
        if (!sys)
            return { success: false, message: 'AI event system not initialized.' };
        if (state === 'on') {
            sys.toggle(true);
    ...
    handleWarp(args) {
        ...
        if (!entityId || !targetWorldId)
            return { success: false, message: 'Usage: warp <entityId> <targetWorldId>' };
        const found = universeRegistry.findEntityAcrossWorlds(entityId);
        if (!found)
            return { success: false, message: `Entity not found across worlds: ${entityId}` };
        if (found.worldId === targetWorldId)
            return { success: false, message: `Entity already in world ${targetWorldId}` };
        const ok = universeRegistry.transferEntity(entityId, found.worldId, targetWorldId);
        if (!ok)
            return { success: false, message: `Warp failed: ${entityId} ${found.worldId} -> ${targetWorldId}` };
        return { success: true, message: `Warped ${entityId}: ${found.worldId} -> ${targetWorldId}` };
    }
    handleDeployDrone(args) {
        ...
        const entity = createEntityByAssemblyWithManager(manager, 'Drone_Observer_001', id);
        const behavior = entity.children[0];
        if (behavior?.components) {
    ...
    handleDrones(args) {
        ...
        const handle = universeRegistry.getWorld(worldId);
        const manager = handle?.manager ?? this.getManager();
        const drones = (manager.entities ?? []).filter((e) => e?.children?.[0]?.components?.mission && e?.children?.[0]?.components?.camera);
        if (drones.length === 0)
            return { success: true, message: `No drones found in world ${worldId}.` };
        const lines = drones.map((e) => {
            const c = e.children[0].components;
            return `- ${e.id} role=${c.identity?.role} mode=${c.mission?.mode} energy=${c.energy?.energy?.toFixed?.(1) ?? c.energy?.energy}`;
        });
        return { success: true, message: `\n📡 [DRONES @ ${worldId}]\n${lines.join('\n')}` };
    }
    handleDroneMission(args) {
        const droneId = args[0];
        const mode = args[1];
        const text = args.slice(2).join(' ');
        if (!droneId || !mode)
            return { success: false, message: 'Usage: drone_mission <droneId> <mode> [text...]' };
        const found = universeRegistry.findEntityAcrossWorlds(droneId);
        if (!found)
            return { success: false, message: `Drone not found: ${droneId}` };
        const behavior = found.entity.children[0];
        const c = behavior?.components;
        if (!c?.mission)
            return { success: false, message: `Entity is not a drone: ${droneId}` };
        c.mission.mode = mode;
    ...
    handleTaxonomy(args) {
        const entityId = args[0];
        if (!entityId)
            return { success: false, message: 'Usage: taxonomy <entityId>' };
        const found = universeRegistry.findEntityAcrossWorlds(entityId);
        if (!found)
            return { success: false, message: `Entity not found: ${entityId}` };
        const c = found.entity.children?.[0]?.components;
        const snapshot = {
    ...
    handleDiseaseStats(args) {
        const worldId = args[0] || this.world.id;
        const handle = universeRegistry.getWorld(worldId);
        const manager = handle?.manager ?? this.getManager();
        const eco = handle?.world?.ecosystemSystem;
        if (!eco?.getDiseaseStats)
            return { success: false, message: `Ecosystem system not active for world ${worldId}` };
    ...
    handleCorpses(args) {
        ...
        const manager = handle?.manager ?? this.getManager();
        const eco = handle?.world?.ecosystemSystem;
        if (!eco?.getCorpseStats)
            return { success: false, message: `Ecosystem system not active for world ${worldId}` };
        const stats = eco.getCorpseStats(manager);
        return { success: true, message: `\n🪦 [CORPSES @ ${worldId}]\n${JSON.stringify(stats, null, 2)}` };
    }
    handleMigrationStats(args) {
        const worldId = args[0] || this.world.id;
        const handle = universeRegistry.getWorld(worldId);
        const eco = handle?.world?.ecosystemSystem;
        if (!eco?.getMigrationStats)
            return { success: false, message: `Ecosystem system not active for world ${worldId}` };
