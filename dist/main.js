...
function assemble(world, manager) {
    const evolutionSystem = new EvolutionSystem();
    const ecosystemSystem = new EcosystemCycleSystem();
    world.ecosystemSystem = ecosystemSystem;
...
function createWorldWithAssembly(worldId, weatherType, manager) {
    ...
    const weatherComp = weatherBehavior.components;
    const weather = weatherComp.weather;
    const world = new World(worldId, {
        assembleManager: manager,
        tickPayloadProvider: () => ({
            environment: {
                soilMoisture: weather.condition === 'Rainy' || weather.condition === 'Stormy'
                    ? 80
